...
class Analyze():
    ...
    def email_network(self):
        email_fromto_list = DataBase("0", "0", "0", "0", "0",
                    "0", "0", "0", "0").select_email_fromto()
        email_from_list = []
        email_to_list = []
        email_edge_list = []
        ...
        DataBase("0", "0", "0", "0", "0",
                    "0", "0", "0", "0").create_emaildegree_table()
        for degree_info in sorted_degree_list:
            email_degree = degree_info[0]
            email_name = degree_info[1]
            DataBase("0", "0", "0", "0", "0",
                        "0", "0", "0", "0").insert_emaildegree(email_degree, email_name)
        # print(sorted(node_degree_list, reverse = True))

    def email_site_search(self):
        email_doubtman = "Isia.Vann"
        email_doubtman_list = DataBase("0", "0", "0", "0", "0",
                    "0", "0", "0", "0").select_email_doubtman(email_doubtman)
        for email in email_doubtman_list:
            print(email)

    def article_site_search(self):
        article_keyword = "Vann"
        article_keyword_list = DataBase("0", "0", "0", "0", "0",
                    "0", "0", "0", "0").select_article_keyword(article_keyword)
