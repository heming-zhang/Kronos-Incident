...
class ParseFile():
        ...
        for file_name in name_list:
            ...
            if len(issue_date) == 0 : 
                ...
                if len(issue_date) == 0 : 
                    ...
                    issue_date = ParseFile("0", issue_date[0][0]).transfer()
            else:
                issue_date = issue_date[0]
            # preserve information in database
            DataBase(file_name, issue_date, report_media, report_title, file_content,
                        "0", "0", "0", "0").insert_article()
    ...
    def filter_email(self):
        email_abstract = DataBase("0", "0", "0", "0", "0",
                    "0", "0", "0", "0").select_email()
        DataBase("0", "0", "0", "0", "0",
                    "0", "0", "0", "0").create_email_table()
        for email in email_abstract:
            ...
            for email_to in email_to_list:
                DataBase("0", "0", "0", "0", "0",
                            email_from, email_to, email_time, email_subject).insert_email()